...
export const setLink = (currentForm, editor) => {
    ...
    if (value !== '') {
        const pendingPromise = new Pending('tiny_teams_link/setLink');
...
const setLinkOnSelection = async(currentForm, editor, url) => {
    ...
    const {html} = await Templates.renderForPromise('tiny_teams_link/embed_link', context);
